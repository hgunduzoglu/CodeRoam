...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  expected_migrations='auth:1,auth:2,device:1,device:2,device:3,integration:1,outbox:1,preview:1,runbook:1,session:1,session:2,session:3,workspace:1,workspace:2,workspace:3'
  ...
  (cd services/control-plane && \
    POSTGRES_TEST_DSN='postgres://postgres:postgres@localhost:5432/coderoam?sslmode=disable' \
      go test -count=1 \
        -run '^(TestAuthorizationIntegration|TestAuthorizationLockIntegration|TestAuthorizationTimeoutIntegration|TestDeviceFingerprintMigrationIntegration|TestRepositoryIntegration|TestRepositoryListPairedDevicesIntegration|TestRepositoryRegisterPairedIntegration|TestRepositoryRegisterPairedTimeoutIntegration|TestRepositoryRegisterPairedConcurrentRetryIntegration)$' \
        ./internal/device)
  (cd services/control-plane && \
    POSTGRES_TEST_DSN='postgres://postgres:postgres@localhost:5432/coderoam?sslmode=disable' \
  ...
  (cd services/control-plane && \
    POSTGRES_TEST_DSN='postgres://postgres:postgres@localhost:5432/coderoam?sslmode=disable' \
      go test -count=1 \
        -run '^(TestPairingAttemptMigrationIntegration|TestPairingAttemptRepositoryIntegration|TestPairingBootstrapServiceIntegration|TestPairingCompletionIntegration|TestRepositoryCreateIntegration|TestServiceStartIntegration)$' \
        ./internal/session)
  (cd services/control-plane && \
    POSTGRES_TEST_DSN='postgres://postgres:postgres@localhost:5432/coderoam?sslmode=disable' \
      go test -count=1 \
        -run '^(TestAgentFingerprintMigrationIntegration|TestRepositoryAgentRevocationIntegration|TestRepositoryAuthorizeAgentIntegration|TestRepositoryAuthorizeAgentLockIntegration|TestRepositoryAuthorizeAgentTimeoutIntegration|TestRepositoryAuthorizeProjectIntegration|TestRepositoryAuthorizeProjectLockIntegration|TestRepositoryListPairedAgentsIntegration|TestRepositoryListProjectsIntegration|TestRepositoryRegisterPairedAgentIntegration|TestRepositoryRegisterPairedAgentTimeoutIntegration|TestRepositoryRegisterPairedAgentConcurrentRetryIntegration)$' \
        ./internal/workspace)
  (cd services/worker && \
    POSTGRES_TEST_DSN='postgres://postgres:postgres@localhost:5432/coderoam?sslmode=disable' \
      go test -count=1 \
